...
void Engine::BeginGame() {
    ...
    int _levelsType[4] = {1, 2, 3, 4};
    int _enemies[4] = {25, 50, 75, 100};
    int _lvl = 0;
    ...
    while ((keyPressSymbol = wgetch(this->mainWindow)) != 'q') {
        ...
        if (player->getHP() > 0) {
            ...
            player->hitByHealthPack(healthPack);
            if (random % 10 == 0) {
                ammoPack->displayAmmoPack(mainWindow);
            }
            if (random % 10 == 0) {
                healthPack->displayHealthPack(mainWindow);
            }
        ...
        mvwprintw(this->intefaceWindow, 6, 4, "Score: %i     Difficulty levels: %i", player->getScore(), _levelsType[_lvl]);
        wrefresh(this->intefaceWindow);
        refresh();
        if (player->getHP() <= 0) {
            mvwprintw(this->intefaceWindow, 2, 4, "Play time: %02d:%02d", lastMin, lastSec);
